			...
			if(in_array($_attribute['attribute_code'], $swatch_attributes)):
				...
				foreach($_attribute['values'] as $value):
					...
					if (count($matches[0]) > 0):
						...
						$jsFunc = 'setLocationConfig(\''.$_product->getProductUrl().'\', \'the-'.$productId.'\')';
						...
						if ($_product->getCjm_useimages() == 1 && $product_image):
							...
							$html = $html.'" onclick="'.$jsFunc.'" />';
						elseif (Mage::helper('colorselectorplus')->getSwatchUrl($theId)):
							$swatchimage = Mage::helper('colorselectorplus')->getSwatchUrl($theId);
							$html .= '<img onMouseOver="listSwitcher(';
							$html .= "this,'".$productId."','".$product_image."','".$attrid."');";
							$html .= '" onclick="'.$jsFunc.'" src="'.$swatchimage.'" id="a'.$attrid.'-'.$theId.'" class="swatch-category" alt="'.$altText.'" width="'.$width.'px" height="'.$height.'px" title="'.$altText.'" />';
						elseif($pos !== false):
							$html .= '<div onMouseOver="listSwitcher(';
							$html .= "this,'".$productId."','".$product_image."','".$attrid."');";
							$html .= '" onclick="'.$jsFunc.'" id="a'.$attrid.'-'.$theId.'" class="swatch-category" style="background-color:'.$color_value.'; width:'.$width.'px; height:'.$height.'px;" title="'.$altText.'">';
							$html .= '</div>';
						else:
