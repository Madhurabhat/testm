...
{	
		...
		if($product_base):
			...
			foreach($images as $key => $image):
				if($product_base['defaultimg'][$key] == 1 && $product_base['morev'][$key] != ''):
					...
					$onloads .= '<li class="onload-moreviews"><a data-easyzoom-source="'.$product_base['image'][$key].'" href="'.$product_base['full'][$key].'" class="zoom-thumbnail" title="'.$product_label.'"><img src="'.$product_thumb.'" width="'.$theSizes['more']['width'].'" height="'.$theSizes['more']['height'].'" alt="'.$product_label.'" title="'.$product_label.'" /></a></li>';	
				elseif($product_base['morev'][$key] == ''):
					$defaultmoreviews = 1;
					$product_image = $product_base['image'][$key];
					$product_thumb = $product_base['thumb'][$key];
					$product_label = $product_base['label'][$key];
					$product_gallery = Mage::helper('colorselectorplus')->getGalUrl($product_base['id'][$key], $_product->getId());
					$defaults .= '<li class="default-moreviews"><a data-easyzoom-source="'.$product_base['image'][$key].'" href="'.$product_base['full'][$key].'" class="zoom-thumbnail" title="'.$product_label.'"><img src="'.$product_thumb.'" width="'.$theSizes['more']['width'].'" height="'.$theSizes['more']['height'].'" alt="'.$product_label.'" title="'.$product_label.'" /></a></li>';
				endif;
			endforeach;
					...
					foreach($options as $option):
						...
						foreach($images as $key => $image):
							if($product_base['morev'][$key] == $value):
								...
								$product_label = $product_base['label'][$key];
								$product_gallery = Mage::helper('colorselectorplus')->getGalUrl($product_base['id'][$key], $_product->getId());
								$html .= '<li class="moreview'.$value.'" style="display:none;"><a data-easyzoom-source="'.$product_base['image'][$key].'" href="'.$product_base['full'][$key].'" class="zoom-thumbnail" title="'.$product_label.'"><img src="'.$product_thumb.'" width="'.$theSizes['more']['width'].'" height="'.$theSizes['more']['height'].'" alt="'.$product_label.'" title="'.$product_label.'" /></a></li>';
							endif;
						endforeach;
               		endforeach;
				endif;
	...
	public function getSwatchHtml($attributeCode, $atid, $_product)
	{
		...
		if($hide == 0) {
			$html = $html.'<div class="swatchesContainerPadded"><ul id="ul-attribute'.$atid.'" class="zoom-gallery">'; }
		else {
			$html = $html.'<div class="swatchesContainer"><ul id="ul-attribute'.$atid.'" class="zoom-gallery">'; }			
		...
		foreach($_color_swatch as $_inner_option_id)
 		{
			...
			if ( count($matches[0]) > 0 )
			{
				...
				//if($zoomenabled && $product_image):
					...
					$zoomStuff = '<a id="anchor'.$theId.'" data-easyzoom-source="'.$product_image.'" href="'.$product_image_full.'" class="zoom-thumbnail" title="'.$frontendlabel.'">';
				//endif;
